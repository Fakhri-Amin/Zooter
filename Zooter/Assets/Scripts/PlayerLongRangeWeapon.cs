...
public class PlayerLongRangeWeapon : MonoBehaviour
{
    ...
    IEnumerator Shoot()
    {
        while (true)
        {
            ...
            float currentAttackDamage = weaponTypeSO.damage + (weaponTypeSO.damage * PlayerStats.Instance.GetAttackDamage() / 100);
            float currentBulletSpeed = weaponTypeSO.bulletMoveSpeed + (weaponTypeSO.bulletMoveSpeed * PlayerStats.Instance.GetBulletSpeed() / 100);

            bullet.Setup(shootDirection, currentAttackDamage, currentBulletSpeed, Bullet.Source.Player);
